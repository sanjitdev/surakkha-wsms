...
| PHA-as-governance, not message-issuance desk (UX-DR from `phases.md`) | UX EXPERIENCE.md §"Foundation" + §"Two-tap confirmation" | AD-12 (closed role enum, 8 entries incl. `utility_message_desk`), AD-17 (PublicNotice channel contract) |
...
- **Rule:** Events of type `PlaybookAmendmentApproved` (when the amendment touches the T3 boundary), `PublicNoticeIssued`, and `PublicNoticeRetracted` (per AD-17) require dual signatures. Both signatures must attest to the **identical `(event_id, payload_hash, version_id)` triple** — the second signature cannot land against a typo-correction v2 after the first attested v1. Implementation uses the linked `SignatureAttestation` pattern (Consistency Conventions row): the second signature emits a `SignatureAttestation` event that references the first and the triple. **Timing model (tightened post-review):** the gateway MUST reject **at write-time** any dual-signature command for which both `SignatureAttestation` events are not already present in the chain; there is no buffered-admission window, no post-hoc admission, no fallback deadline. A half-attested issuance attempt produces a typed `CommandRejected{reason: dual_signature_missing}` event and **no chain event for the underlying command** is written. The gateway accepts the underlying command only after both attestations with matching triples are recorded. The two signatures identify distinct actor identities per the closed role enum of AD-12 (vendor / pha_approver for schema bumps per AD-4 / AD-15; pha_approver / utility_operator for playbook promotion per AD-13; **pha_approver / utility_message_desk** for `PublicNoticeIssued` and `PublicNoticeRetracted` per AD-17 — note: `utility_message_desk`, not `utility_operator`, since issuance is the named on-call role distinct from day-to-day operations; shift-coverage via `ShiftCoverageActivated` per AD-17 rule 9 is the only way a `utility_operator` may issue under the dual-signature rule).
...
- **Rule:** Every event carries an `actor_identity` field with two sub-fields: `role` (closed enum, per-city-tenant matrix: `{vendor, pha_approver, utility_operator, utility_message_desk, anjali, priya, pha_viewer, system}` — strictly this set, never a free-form string; **8 entries** as of 2026-09-07) and `ref` (a unique actor reference resolved from the auth-service session token, **not** minted by the emitting component). The gateway **rejects events whose `role` is not in the closed enum or whose `ref` cannot be resolved to a live auth-session reference** — this is enforced at write-rejection, not as a soft check. The vendor's liability boundary ends at events the vendor's actors emitted; the PHA's liability boundary covers PHA-actor events; the utility covers utility-actor events; the **`utility_message_desk` role** is the named on-call operator who issues T3+ consumer messages and is **distinct** from `utility_operator` (the day-to-day operator role) — its liability boundary is the issuance act itself (`PublicNoticeIssued`, `PublicNoticeRetracted`), while `pha_approver` retains the dual-signature attestation responsibility on the `(event_id, payload_hash, version_id)` triple (AD-11). **Critically: `pha_approver` does not include message-issuance rights.** The audit chain is the evidence surface for the split; the closed enum + session-bound ref make the split structural, not contractual.
...
### AD-17 — Public Notice channel contract (issuance + retraction)

- **Binds:** AD-1 (event store = audit chain), AD-11 (two-person rule on dual-signature events), AD-12 (closed role enum incl. `utility_message_desk`), AD-14 (idempotency); FR-7 (Ramesh consumer messaging); C-4 (consumer-message dual-signature), C-11 (SMS first-class, dual-channel), C-13 (vendor / PHA / utility liability split on consumer messaging); UX EXPERIENCE.md §"Two-tap confirmation" + §"Voice and tone — Message string structure" + §"Councillor channel hostile-fallback".
- **Prevents:** scripted single-actor T3 spam (UX adversarial §8); false-T3 recall drift caused by treating "Safe now" as a retraction (§2 critical gap); the councillor voice-message becoming an uncontrolled broadcast surface with no audit hook (§7 high gap); the PHA-as-message-desk contradiction when issuance affordance lives on PHA-pane (§4 critical gap); single-character Bangla typo risk on template-free composition (no free composition — chain fills the slots).
- **Rule:**
  1. **Event types** — `PublicNoticeIssued` (issuance) and `PublicNoticeRetracted` (false-alarm / recall correction). Both are first-class event types; retraction is **not** a parameter on issuance. The sixth message shape ("Retraction / false alarm") in EXPERIENCE.md is bound to the `PublicNoticeRetracted` event type; the other five shapes bind to `PublicNoticeIssued`.
  2. **Dual-signature precondition** — `PublicNoticeIssued` requires two `SignatureAttestation` events (one from `pha_approver`, one from `utility_message_desk`) referencing the **identical `(event_id, payload_hash, version_id)` triple** (per AD-11). The gateway accepts the issuance command only after both attestations are recorded. `PublicNoticeRetracted` follows the same dual-signature rule — retracting a notice is itself a politically and legally sensitive act (false-alarm admission).
  3. **Retraction precondition** — the gateway **rejects `PublicNoticeRetracted` events whose referenced prior `PublicNoticeIssued` `event_id` is not present in the chain for the tenant**. The chain is the authority — not the projection. A retraction without an issuance is structurally impossible. **Critical clarification (tightened post-review):** a `SubjectRedacted` tombstone (per AD-16) on the prior `PublicNoticeIssued` does **not** block retraction; the prior issuance remains chain-present even when its payload is projection-redacted, and the retraction event itself becomes the chain-anchored evidence that an admission occurred (the RTBF admission trail).
  4. **Rate limit** — gateway-enforced: max **5 `PublicNoticeIssued` confirmations per `utility_message_desk` actor per rolling 60 minutes**. Beyond the limit, the issuance command is rejected with `PublicNoticeRateLimitExceeded` and the UI surfaces "Rate limit reached (5/hour) — escalation requires City Incident Commander sign-off." Defeats scripted single-actor automation; preserves human acute response. `PublicNoticeRetracted` is exempt from this limit (retraction is a corrective act and must not be rate-blocked).
  5. **Template character budget (tightened post-review)** — the gateway enforces the character budget **at write-time**, before any `PublicNoticeIssued` event is committed to the chain. EN locale (GSM-7 alphabet): ≤ 160 characters per filled message. BN locale (UCS-2): ≤ 70 characters per filled message. Multi-segment messages (153 × N) are rejected; one SMS is the hard cap. Build-time test fills each template slot with worst-case strings (longest zone name, longest Bangla transliteration, full 6-word contaminant class) and asserts the filled string is within budget; failing templates block the playbook amendment that introduced them. A template that exceeds budget produces `CommandRejected{reason: TemplateExceedsCharacterBudget}` and **no chain event for the issuance is written**. Post-chain-commit partial-dispatch failure (one channel rejects after the other has accepted) emits `ChannelDeliveryFailed{channel, notice_event_id}` but does **not** roll back the issuance — `PublicNoticeRetracted` is the corrective path.
  6. **Dual-channel dispatch** — every `PublicNoticeIssued` is dispatched on WhatsApp **and** SMS (signed gateway per C-11); this is the consumer-side equivalent of the two-person rule — even if one channel is unreachable, the other carries the notice. A failed delivery retries and escalates to the alternative channel (NFR-R1.3).
  7. **Councillor-routed voice** — **deliberate exception** to template discipline. The councillor's voice is the persona win (UX-DR5) and the trust-bridge; the chain does not compose the councillor's message. What the chain **does** capture is the **endorsement event itself**: every councillor voice-relay emits a `CouncillorEndorsementRecorded` event referencing the `PublicNoticeIssued` `event_id`, the ward, the councillor's actor_ref, and the `occurred_at` of the relay. The system records that endorsement happened; it does not record the content. This is not surveillance of the councillor — it is a chain-anchored record that ward-level endorsement occurred, which defends Anjali and the platform against a "no one told us" defense.
  8. **Councillor-voice monitoring + hostile fallback** — the councillor channel is monitored continuously. The gateway SLA monitor emits `CouncillorRelayMissed{ward, sla_window, occurred_at}` when a councillor fails to relay within 15 min of dispatch (the SLA band) and emits `CouncillorQualityFlagged{ward, pattern, evidence_hash}` when relay quality degrades (audible refusal-pattern matches from logged transcripts). The PHA-pane aggregate tiles surface both event types per ward. **Route-around for hostile councillors** — a ward whose councillor is hostile or unreachable is not blocked from notification. The ward receives WhatsApp + SMS + a PHA-direct citation channel (text message from the Director's office, in the Director's name). **Sole emitter (tightened post-review):** the gateway SLA monitor is the **sole emitter** of `CouncillorBypassActivated{reason: sla_breach | quality_flag | manual, fallback_channel: pha_direct, triggered_by_actor_ref, occurred_at}` and the manual-PHA-activation path is via PHA-pane (`actor_identity.role: pha_approver`). SLA-breach bypass requires a prior `CouncillorRelayMissed` in the chain; quality-flag bypass requires a prior `CouncillorQualityFlagged`; manual bypass requires PHA-pane operator attestation and is logged with the actor_ref. The citizen gets the notice without the trust-bridge penalty; the trust-bridge penalty falls on the councillor (visible on PHA-pane aggregate tiles as a delivery-metric flag).
  9. **Issuance surface** — the two-tap confirmation lives on **Priya-desktop**, **not** on PHA-pane. PHA-pane surfaces the *approval* (the `SignatureAttestation` events on the triple), not the issuance affordance. Anjali-mobile never surfaces issuance. The natural issuer is the named on-call `utility_message_desk` actor. **Shift-coverage exception (tightened post-review):** a `utility_operator` actor MAY issue T3+ consumer messages **only if** a chain-anchored `ShiftCoverageActivated{covered_role: utility_message_desk, named_desk_actor_ref, covering_operator_ref, valid_until, activated_by_actor_ref}` event is present in the chain and is currently unexpired (`occurred_at + valid_until > now`). The gateway MUST reject otherwise with `CommandRejected{reason: shift_coverage_missing_or_expired}`. The complementary `ShiftCoverageEnded{reason: shift_end | relieved | emergency_handoff}` event closes the coverage window. Both event types are first-class; coverage windows are append-only and audit-replayable.
  10. **Shoulder-surfing redaction at preview** — the T3+ two-tap modal previews the message in a redacted form by default (`{RECIPIENT_LIST}` collapsed to count, `{REASON_LANG_FIELD}` substituted for false-alarm cause); a 750 ms hold-gesture ("Reveal full") exposes the full message. Redaction lifts at confirm; the chain event itself is fully auditable.
...
| **Public Notice channel** | `PublicNoticeIssued` and `PublicNoticeRetracted` are first-class event types (per AD-17). Retraction requires the prior issuance in the chain (not the projection — `SubjectRedacted` does not block retraction, per AD-16 reconciliation); both require dual `SignatureAttestation` from `pha_approver` + `utility_message_desk` at write-time (per AD-11 — no buffered admission). Gateway-enforced character budget (EN ≤160 GSM-7, BN ≤70 UCS-2) at write-time, before chain commit; post-commit partial-dispatch failure emits `ChannelDeliveryFailed` and is corrected via `PublicNoticeRetracted`. Gateway-enforced rate limit: 5 issuances / 60 min / actor (retraction exempt — see Deferred for oscillation-vector follow-up). WhatsApp + SMS dual-channel dispatch per C-11. Councillor voice is a deliberate trust-bridging exception — endorsement is logged via `CouncillorEndorsementRecorded`, not the content. Councillor-hostile fallback: SLA monitor is sole emitter of `CouncillorBypassActivated`, with `CouncillorRelayMissed` / `CouncillorQualityFlagged` as preconditions; manual bypass via PHA-pane pha_approver attestation. Shift coverage: `ShiftCoverageActivated{covered_role: utility_message_desk, covering_operator_ref, valid_until}` is the chain-anchored bridge that lets a `utility_operator` substitute for the named on-call `utility_message_desk` (per AD-17 rule 9). |
...
| FR-7 consumer messaging (Ramesh) | `adapters/channel/` + `components/playbook/` | AD-3, AD-11, AD-12 (`utility_message_desk` role), AD-17 (PublicNotice channel contract) |
| FR-7 retraction / false-alarm correction | `adapters/channel/` + `components/playbook/` | AD-17 (Retraction precondition + dual-signature on retraction) |
...
| **Bangla consumer-message copy + literacy-floor review** | UX adversarial §6: Bangla copy absent from spine; "Do not drink" → "please drink" single-char deletion is the canonical typo risk; `{CLASS}` slot must come from a pre-approved glossary, not free text; verification horizon must use a single digit form per locale; Bangla copy must be tested on ≥ 3 low-literacy readers per shape before any template ships | First Anjali-mobile / Ramesh-channel engineering hire; pre-launch gate on the message-template build-time test |
| **Councillor-endorsement content logging policy** | AD-17 records the *event* of endorsement, not the content. Whether to additionally capture transcript hashes (e.g., SHA-256 of councillor voice recording) for forensic defense is a v2 policy call | First hostile-councillor incident; PHA legal review |
| **`utility_message_desk` shift-coverage fallback** | AD-17 names the role. The policy for what happens when the named `utility_message_desk` is unreachable on shift (does `utility_operator` always cover? Does City Incident Commander?) is an operational spec, not an architectural invariant. AD-17 rule 9 now requires `ShiftCoverageActivated` as the chain-anchored bridge — the policy shape is settled, the operational scenarios are not | First PHA operational drill; OQ on City Incident Commander sign-off path |
| **PublicNoticeOscillation limit** (adversarial Finding 6, medium) | AD-17 rule 4 exempts `PublicNoticeRetracted` from the 5/hr/actor issuance rate limit on the grounds that retraction is a corrective act and must not be rate-blocked. The exemption creates an issue/retract/issue/retract oscillation vector that pollutes the audit trail at 10 politically-charged chain events per hour. A paired-rate rule (issuance + retraction ≤ 7 per 60 min per actor) would close the gap. **Deferred** — not safety-critical (retraction is bounded by issuance); the WB evidence export consumes the audit trail linearly and can filter if needed; revisit after first operational quarter | First operational data on retraction frequency; v2 hardening pass |
| **KurrentDB v26.2 licensing re-check** (web-verification Finding 1, high) | Event-store Stack row cites KurrentDB v25.x; v26.2 released Aug 2026 reassigns clustering, Raft, RO replicas, archiving, and encryption-at-rest to Enterprise license. The warm-standby topology (AD-9) likely crosses the new Enterprise threshold. **Deferred** to stack-selection pass during `bmad-create-epics-and-stories` | Stack-selection pass; WB procurement cycle |
| **Bangladesh-resident IaaS provider availability** (web-verification Finding 4, high) | AD-9 names Pathao Cloud, TigerIT Bangladesh cloud, government cloud as candidates. Web verification surfaced that Pathao is a consumer super-app on MongoDB Atlas / GCP, not a domestic IaaS offering; no public confirmation of Tier-III BD-resident IaaS from the named providers in 2026. Concrete options: BDix-hosted facilities, Summit Communications, BTS, BTCL. **Deferred** — procurement reality, not architecture | A8 sub-task validation; first WB procurement cycle |
| **WhatsApp utility-template cost/throughput re-estimate** (web-verification Finding 2, medium) | Meta's Oct 1, 2026 pricing change begins charging for all Service (free-form) messages; utility-template path (T3+ public notice) remains intact. Per-T3-event × ward-count cost math needs a fresh estimate using current utility-template volume tiers and the chosen BSP (Twilio/360dialog/Infobip). **Deferred** — operational, not architectural | First BSP selection pass; A8 cost-forecast task |
| **Dual-channel dispatch primary/fallback ladder** (web-verification Finding 8, medium) | AD-17 rule 6 specifies WhatsApp + SMS dispatch but does not document the failure-mode ordering: which channel is primary, the fallback SLA, the total-failure escalation (e.g., loudspeaker, ward councillor activation). **Deferred** — operational, not load-bearing on the architectural invariants | First operational drill with PHA + utility on-call |
| **Open-questions / `[ASSUMPTION]` tagging in memlog** (rubric Finding 13, partial) | Reviewer-gate rubric walker flagged that the memlog does not surface `[ASSUMPTION]`-tagged entries as a discrete section. AD-17 introduces several assumptions worth tagging (15-min councillor SLA, 5/hr/actor rate limit, single-SMS hard cap). **Deferred** to a memlog cleanup pass | Next architecture amendment; memlog cleanup pass |
